...
export class reconcilationreportComponent implements OnInit {
          ...
          if (this.model.ReportType == "Comm") {
            ...
            if (this.model.sORd == "S") {
              if (this.model.exporttype == "StateWise") {
                this.stateData1 = this.utilService.convertToResultArray(
                  response[0]
                );
              }
              if (this.model.exporttype == "RegionWise") {
                this.regionData1 = this.utilService.convertToResultArray(
                  response[0]
                );
              }
              if (this.model.exporttype == "LocationWise") {
                this.branchData1 = this.utilService.convertToResultArray(
                  response[0]
                );
              }
              if (this.model.exporttype == "EmployeeWise") {
                this.employeeData = this.utilService.convertToResultArray(
                  response[0]
                );
              }
            ...
            if (this.model.sORd == "D") {
              ...
              this.ZoneData1 = this.utilService.convertToResultArray(
                response[0]
              );
  ...
  showPolicyDashboard(data, head): void {
    if (
      (head == "PolicyNo" && this.model.ReportType == "IssuanceOnly") ||
      this.model.ReportType == "IssuanceAvailableandLoginMissing"
    ) {
      const modal = this.modalService.create({
        nzTitle: "Policy Details",
        nzContent: PolicyDetailsComponent,
    ...
    this.dataServ
      ...
      .then((response) => {
        ...
        if (response && response[0]) {
          ...
          if (
            this.model.exporttype == "EmployeeWise" &&
            this.model.sORd == "S"
          ) {
