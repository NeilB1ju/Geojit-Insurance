...
export class InwardEntryComponent implements OnInit {
  ...
  Search() {
    ...
    reqParams = {
      ...
      detailArray: [
        {
          SEARCHTYPE: FindType.InwardEntrySearch,
          INITIALSEARCH: "N",
          WhereClause: "",
          LANGID: 1,
        },
    ...
    this.dataServ
      ...
      .then((response) => {
        ...
        if (response && response[0] && response[0].rows.length > 0) {
          ...
          this.model.client = {
            tradecode: res[0].Tradecode,
            ClientFirstName: res[0].ClientFirstName,
            ClientID: res[0].ClientID,
          };
          let inwardType =
            res[0].LGType == "E"
              ? { EmployeeID: res[0].LGCode, Name: res[0].name }
              : { Location: res[0].Location, LocationName: res[0].name };
          this.model.inwardType = inwardType.EmployeeID
            ? {
                EmployeeID: inwardType.EmployeeID,
                Name: inwardType.Name,
                Code: inwardType.EmployeeID,
              }
            : "";
          this.model.FinwardType = inwardType.Location
            ? { Location: inwardType.Location, LocationName: inwardType.Name }
            : "";
          this.model.BA = this.model.inwardType.Location
            ? this.model.inwardType.Location
            : "";
          this.model.pdtCategory = res[0].CatID
            ? { CategoryID: res[0].CatID, CategoryCode: "", CategoryName: "" }
            : "";
  ...
  Save(form) {
    ...
    if (this.Drop1 == "P") {
      ...
      if (
        ...
        this.model.pdtCategory == null ||
        this.model.pdtCategory == undefined
      ) {
        ...
        detailArray: [
          {
            ...
            ProductCat: this.model.pdtCategory
              ? this.model.pdtCategory.CategoryID
              : 0,
            // "LGCode": this.model.type == 'E' ? this.model.inwardType.EmployeeID : this.model.inwardType.Location,

            LGCode:
              this.model.type == "E"
                ? this.model.inwardType.EmployeeID
                : this.model.type == "F"
                ? this.model.FinwardType.Location
                : this.model.BA,
            EntryDate: this.model.entryDate
              ? moment(this.model.entryDate).format(
    ...
    if (data) {
      if (!this.formHdlr)
        if (this.model.applNo)
          ...
          //   this.model.client = '';
          this.model.applNo = "";
      if (this.model.product) this.model.product = this.getProduct(1);
      if (this.model.entryDate) this.model.entryDate = new Date();
      if (this.model.inwardType) this.model.inwardType = "";
    }
  }
  ...
  getProductcat() {
    this.dataServ
      .getResponse({
        ...
        detailArray: [
          {
            CatId: this.model.pdtCategory.CategoryID
              ? this.model.pdtCategory.CategoryID
              : 0,
            Euser: this.currentUser.userCode,
          },
      ...
      .then((response) => {
        ...
        if (response && response[0]) {
          ...
          if (catlist)
            this.model.pdtCategory = catlist[0].CategoryID
              ? {
                  CategoryID: catlist[0].CategoryID,
                  CategoryCode: catlist[0].CategoryCode,
                  CategoryName: catlist[0].CategoryName,
                }
              : "";
        } else {
        }
