...
export class CommonReportsComponent implements OnInit {
  ...
  validatationForm() {
    ...
    PolicyNumber_Or_ApplicationNo : [null, [ Validators.pattern("^[0-9a-zA-Z\-\/]*$")]],
    Report_Type: [null],
    Insurer_Type: [null],
  ...
  preview() {
    ...
    let isValid = this.ValidServ.validateForm(this.validateForm, this.clientFormControlNames)
    if(isValid==false){
        ...
        if (response && response[0] && response[0].rows.length > 0) {
          ...
          if (this.model.sORd == 'S') {
            if (this.model.exporttype == 'StateWise') {
              this.stateData1 = this.utilServ.convertToResultArray(response[0]);
            ...
            if (this.model.exporttype == 'RegionWise') {
              this.regionData1 = this.utilServ.convertToResultArray(response[0]);
            ...
            if (this.model.exporttype == 'LocationWise') {
              this.branchData1 = this.utilServ.convertToResultArray(response[0]);
            ...
            if (this.model.exporttype == 'EmployeeWise') {
              this.EmployeeData1 = this.utilServ.convertToResultArray(response[0]);
            ...
            if (this.model.ReportType == 'Issuance' || this.model.ReportType == 'LogVsIssu') {
              if (this.model.exporttype == 'TreeView') {
  ...
  exportData() {
    ...
    this.dataServ.getResponse({
      ...
      if (this.model.exporttype == 'TreeView' && this.model.sORd == 'S') {
        ...
        this.utilServ.Excel(this.Columns, this.Records, this.model.exporttype)
        this.isSpinning = false
      }
      else {
        let res;
  ...
  getInsuranceCompany() {
    ...
    this.ProductCategoryFindopt = {
      ...
      descColumn: 'CategoryName',
      descLabel: 'CategoryName',
      whereClause: this.model.InsurerType ? "ProductTypeId = '" + this.model.InsurerType + "'" : '1=1',
      hasDescInput: false,
      requestId: 2
    }
  }
