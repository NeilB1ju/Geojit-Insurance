...
export class IRDARegisterComponent implements OnInit {
    ...
    this.dataServ
      ...
      .then((response) => {
        if (response && response[0] && response[0].rows.length > 0) {
          ...
          this.model.Location = {
            Location: this.dataServ.branch ? this.dataServ.branch : "",
          };
          ...
          this.model.spCode = {
            SPCode: data[0].Spcode ? data[0].Spcode : "",
            Active: data[0].SpStatus ? "Y" : "N",
            Name: data[0].Spname ? data[0].Spname : "",
          };
  ...
  onChangeStatus(data: any) {
    ...
    data === "Close"
      ? (this.closeDateFlag = true)
      : (this.closeDateFlag = false);
  }
  ...
  Save(type): void {
    ...
    if (
      ...
      (this.model.PolicyNumber.PolicyNo === "" ||
        this.model.PolicyNumber.PolicyNo === undefined)
    ) {
    ...
    if (
      this.closeDateFlag == true &&
      (this.model.closedDate == null || this.model.closedDate == undefined)
    ) {
              ...
              detailArray: [
                {
                  ...
                  Euser: this.currentUser.userCode
                    ...
                    : "",
                  Nameofcomplainant: this.model.nameOfComplaint
                    ? this.model.nameOfComplaint
                    : "",
                  Natureofcomplaint: this.model.natureOfComplaint
                    ? this.model.natureOfComplaint
                    : "",
                  Date: this.model.dateOfComplaint
                    ? moment(this.model.dateOfComplaint).format(
                        AppConfig.dateFormat.apiMoment
                      )
                    : "",
                  PolicyNumber: this.model.PolicyNumber.PolicyNo
                    ? this.model.PolicyNumber.PolicyNo
                    : "",
                  Spcode: this.model.spCode.SPCode
                    ? this.model.spCode.SPCode
                    : "",
                  Spname: this.model.spName ? this.model.spName : "",
                  SpStatus: this.model.spStatus ? this.model.spStatus : "",
                  ActionTaken: this.model.actionTaken
                    ? this.model.actionTaken
                    : "",
                  complaintStatus: this.model.Status ? this.model.Status : "",
                  ClosedDate: this.model.closedDate
                    ? moment(this.model.closedDate).format(
                        AppConfig.dateFormat.apiMoment
                      )
                    : "",
  ...
  View() {
    ...
    if (this.img) {
      this.imgType === "image/jpg" &&
        this.previewFile(this.img, "image/jpg", this.fileNameFromDb);
      this.imgType === "image/jpeg" &&
        this.previewFile(this.img, "image/jpeg", this.fileNameFromDb);
      this.imgType === "application/pdf" &&
        this.previewFile(this.img, "application/pdf", this.fileNameFromDb);
    }
  }
