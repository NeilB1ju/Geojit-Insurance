...
export class TransactionFileUploadComponent implements OnInit {
  ...
  uploadFiles() {
    ...
    for (var i = 0; i < this.clientList.length; i++) {
      ...
      if (el.checked) {
        isChecked = true;
        if (el.file.length == 0) {
          this.notif.warning("Please choose file for " + el.FileDescription, '');
          return;
        }
      }
    }
  ...
  processUpload(i) {
    ...
    this.processFile(i).then((response) => {debugger
      ...
      this.updateData(filename, i).then((response) => {debugger
  ...
  private updateData(fileName, i) { debugger
      ...
      this.dataServ.getResponse({
        ...
        "detailArray":
          [{
            ...
            TranDate: moment(this.model.tranDate).format(AppConfig.dateFormat.apiMoment),
